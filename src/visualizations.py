...
def create_visualizations(train_df: pd.DataFrame, mapping_df: pd.DataFrame):
    ...
    output_dir = PROJECT_ROOT / 'out' / 'mmim'
    ...
    train_df['SubjectName'].value_counts().nlargest(10).plot(kind='bar')
    ...
    corr_matrix = train_df[['ConstructId', 'SubjectId'] + misconceptions].corr()
    ...
    wordcloud = WordCloud(width=800, height=400, background_color='white').generate(' '.join(mapping_df['MisconceptionName']))
